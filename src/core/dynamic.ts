...
const buildDynamicReplacementQueries = (state: State[], attribValues: AttributeData, ignoreEmpty: boolean) => {
    ...
    if (goal) {
      if (undefined === value) {
        if (dependencies && dependencies.length > 0) {
          const dependenciesKnown = dependencies.every((dep) => Object.hasOwn(knownValues, dep));
  ...
  const unKnownValuesFin = unKnownValues.filter((it) => !Object.hasOwn(knownValues, it.goal!));
 ...
 */
export const buildDynamicReplacements = async (state: State[], attribValues: AttributeData, api: AxiosInstance, project: ProjectId, release: ReleaseId, sessionId: SessionId): Promise<AttributeData> => {
